...
function readError(cause: unknown, fallback: string): string {
  return cause instanceof Error ? `${fallback} 请检查课程归属、版本状态或服务连接后重试。` : fallback;
}
...
export function TeacherSyllabus() {
  ...
  const [prefilledTypedContent, setPrefilledTypedContent] = useState<TypedSyllabusContent | null>(null);
  ...
  const syllabusAssist = useTeacherFormAssist(courseId, 'syllabus_candidate');

  const loadCatalog = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      const courseResponse = await fetchTeacherProductionCourses();
      ...
      setError(readError(cause, '无法读取本人课程。'));
    } finally {
      setLoading(false);
    }
  }, [requestedCourseId]);
  ...
  useEffect(() => {
    ...
    const version = versions.find((item) => item.id === requestedVersionId);
    if (!version) {
      setRouteSelection(searchParams, setSearchParams, 'version', '');
      return;
    }
    if (preview?.id !== version.id) setPreview(version);
  }, [preview?.id, requestedVersionId, searchParams, setSearchParams, versions]);

  useEffect(() => {
    if (preview && requestedVersionId !== preview.id) {
      setRouteSelection(searchParams, setSearchParams, 'version', preview.id);
    }
  }, [preview, requestedVersionId, searchParams, setSearchParams]);
  ...
  const closePreview = () => {
    setPreview(null);
    setRouteSelection(searchParams, setSearchParams, 'version', '', false);
  };

  const applySyllabusPrefill = async () => {
    const context = await syllabusAssist.apply();
    if (!context) return;
    const draft = context.draft as SyllabusPrefill;
    const typed = draft.typed_content;
    if (!typed || typed.modules.length === 0) {
      setError('当前课程没有足够的知识点，无法填入 typed syllabus 草稿。');
      return;
    }
    const firstModule = typed.modules[0];
    setPrefilledTypedContent(typed);
    setForm({
      title: typed.title,
      summary: typed.summary,
      learningOutcomes: typed.learning_outcomes.join('\n'),
      moduleId: firstModule.module_id,
      moduleTitle: firstModule.title,
      knowledgeNodeIds: firstModule.knowledge_node_ids[0] ?? '',
      moduleOutcome: firstModule.learning_outcome,
      activities: firstModule.activities.join('\n'),
      assessmentPlan: typed.assessment_plan,
      sourceNote: typed.source_note,
      reason: draft.reason ?? '',
    });
    setGenerationAgentRunId(draft.agent_run_id ?? '');
    setGenerationEvidenceId(draft.evidence_snapshot_id ?? '');
    setGenerationReason(draft.reason ?? '');
  };
  ...
  const buildTypedContent = (): TypedSyllabusContent | null => {
    ...
    if (prefilledTypedContent) {
      return {
        ...prefilledTypedContent,
        title: form.title.trim(),
        summary: form.summary.trim(),
        learning_outcomes: learningOutcomes,
        modules: prefilledTypedContent.modules.map((module, index) => index === 0 ? {
          ...module,
  ...
  const generate = async () => {
    if (!courseId || !generationAgentRunId.trim() || !generationEvidenceId.trim() || !generationReason.trim()) {
      setError('生成候选必须选择已完成且关联有效的运行与 Evidence，并填写理由。');
      return;
    }
    setSaving(true);
    setError(null);
  ...
  return (
    ...
    >
      ...
      <section className="mt-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><label className="grid max-w-xl gap-1 text-xs text-slate-600">本人课程<select value={courseId} onChange={(event) => selectCourse(event.target.value)} className="min-w-0 rounded-lg border border-slate-200 px-3 py-2 text-sm">{courses.length === 0 && <option value="">暂无本人课程</option>}{courses.map((course) => <option key={course.id} value={course.id}>{course.code} · {course.title}</option>)}</select></label>{selectedCourse && <p className="mt-3 rounded-lg bg-brand-blue-50 px-3 py-2 text-xs text-brand-blue-900">知识点、运行和 Evidence 均通过当前课程的可读选择器使用；内部标识只在提交时由客户端携带给原有校验。</p>}</section>
      <TeacherFormAssistPanel purpose="syllabus_candidate" context={syllabusAssist.context} loading={syllabusAssist.loading} applying={syllabusAssist.applying} error={syllabusAssist.error} allowDraftWithoutDependency onApply={() => void applySyllabusPrefill()} />

      <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
        <section className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm"><div><h2 className="text-sm font-semibold text-slate-800">人工编辑 typed 版本</h2><p className="mt-1 text-xs leading-5 text-slate-500">推荐草稿含四个模块；教师可在提交前修改总体目标、首个模块、活动、评估和来源说明，服务端仍会校验知识点归属。</p></div>{prefilledTypedContent && <p className="mt-3 rounded-lg bg-emerald-50 px-3 py-2 text-xs text-emerald-900">已填充 {prefilledTypedContent.modules.length} 个结构化模块；其余模块会在预览中保留，点击“填充推荐内容”可恢复推荐值。</p>}<div className="mt-3 grid gap-2 md:grid-cols-2"><input value={form.title} onChange={(event) => setForm((current) => ({ ...current, title: event.target.value }))} placeholder="大纲标题" className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /><input value={form.moduleId} onChange={(event) => setForm((current) => ({ ...current, moduleId: event.target.value }))} placeholder="模块逻辑键" className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /><input value={form.moduleTitle} onChange={(event) => setForm((current) => ({ ...current, moduleTitle: event.target.value }))} placeholder="模块标题" className="rounded-lg border border-slate-200 px-3 py-2 text-xs" /><select aria-label="选择本课程知识点" value={form.knowledgeNodeIds} onChange={(event) => setForm((current) => ({ ...current, knowledgeNodeIds: event.target.value }))} className="rounded-lg border border-slate-200 px-3 py-2 text-xs"><option value="">选择本课程知识点</option>{syllabusAssist.context?.knowledge_points.map((node) => <option key={node.id} value={node.id}>{node.label}</option>)}</select></div><textarea value={form.summary} onChange={(event) => setForm((current) => ({ ...current, summary: event.target.value }))} placeholder="课程摘要" rows={3} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><textarea value={form.learningOutcomes} onChange={(event) => setForm((current) => ({ ...current, learningOutcomes: event.target.value }))} placeholder="学习目标，每行一项" rows={3} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><textarea value={form.moduleOutcome} onChange={(event) => setForm((current) => ({ ...current, moduleOutcome: event.target.value }))} placeholder="模块学习产出" rows={2} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><textarea value={form.activities} onChange={(event) => setForm((current) => ({ ...current, activities: event.target.value }))} placeholder="学习活动，每行一项" rows={2} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><textarea value={form.assessmentPlan} onChange={(event) => setForm((current) => ({ ...current, assessmentPlan: event.target.value }))} placeholder="评估计划" rows={2} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><textarea value={form.sourceNote} onChange={(event) => setForm((current) => ({ ...current, sourceNote: event.target.value }))} placeholder="来源与证据说明" rows={2} className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><input value={form.reason} onChange={(event) => setForm((current) => ({ ...current, reason: event.target.value }))} placeholder="人工编辑理由（必填，写入审计）" className="mt-2 w-full rounded-lg border border-slate-200 px-3 py-2 text-xs" /><button type="button" disabled={saving || !courseId} onClick={() => void createManual()} className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-brand-blue-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50"><FilePlus2 className="h-3.5 w-3.5" />{saving ? '正在保存…' : '保存待审核版本'}</button></section>
        <section className="rounded-2xl border border-violet-100 bg-violet-50/40 p-4 shadow-sm"><div><h2 className="text-sm font-semibold text-violet-950">从已完成运行生成候选</h2><p className="mt-1 text-xs leading-5 text-violet-900">前端不直接调用 Provider；仅选择当前课程中已完成、已关联且包含 typed 输出的运行与 Evidence，证据不足会被确定性拒绝。</p></div><div className="mt-3 grid gap-2"><select aria-label="选择已完成运行与关联 Evidence" value={`${generationAgentRunId}:${generationEvidenceId}`} onChange={(event) => { const pair = syllabusAssist.context?.agent_evidence_pairs.find((item) => `${item.agent_run_id}:${item.evidence_snapshot_id}` === event.target.value); setGenerationAgentRunId(pair?.agent_run_id ?? ''); setGenerationEvidenceId(pair?.evidence_snapshot_id ?? ''); }} className="rounded-lg border border-violet-200 bg-white px-3 py-2 text-xs"><option value=":">选择已完成运行与关联 Evidence</option>{syllabusAssist.context?.agent_evidence_pairs.map((pair) => <option key={`${pair.agent_run_id}:${pair.evidence_snapshot_id}`} value={`${pair.agent_run_id}:${pair.evidence_snapshot_id}`}>{pair.label} · {pair.summary.slice(0, 56)}</option>)}</select><textarea value={generationReason} onChange={(event) => setGenerationReason(event.target.value)} placeholder="生成候选理由" rows={3} className="rounded-lg border border-violet-200 bg-white px-3 py-2 text-xs" /></div><button type="button" disabled={saving || !courseId || !generationAgentRunId || !generationEvidenceId} onClick={() => void generate()} className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-violet-600 px-3 py-2 text-xs font-medium text-white disabled:opacity-50"><Sparkles className="h-3.5 w-3.5" />从已完成运行生成候选</button></section>
      </div>
