...
const cryptoEvidence: CourseMockEvidence = [
  {
    ...
    asset_type: 'pdf',
    excerpt: 'SHA-2 系列输出长度固定，单向哈希不可逆，适合做完整性校验但不适合直接存储用户密码。',
    page_no: 5,
    chapter: '哈希算法基础',
    timestamp: null,
    reliability: 0.95,
  ...
  },
  {
    ...
    rights_note: 'CC BY-SA 4.0',
    asset_type: 'web_article',
    excerpt: '用户密码应当使用 bcrypt / argon2 等带 salt 的慢哈希；直接使用 MD5 / SHA-1 是已知误用。',
    page_no: null,
    chapter: '密钥与密码管理',
    timestamp: null,
    reliability: 0.9,
  ...
  },
  {
    ...
    excerpt: 'AES-GCM 自带认证标签，比 AES-CBC + HMAC 更不容易写错；高并发场景下注意 nonce 唯一性。',
    page_no: null,
    chapter: 'AES 模式选择',
    timestamp: null,
    reliability: 0.72,
...
const networkEvidence: CourseMockEvidence = [
  {
    ...
    asset_type: 'web_article',
    excerpt: '主动扫描的 TCP SYN 半连接最常用；UDP 扫描需要结合服务指纹和差速重试。',
    page_no: null,
    chapter: '端口扫描技术',
    timestamp: null,
    reliability: 0.93,
  ...
  },
  {
    ...
    asset_type: 'web_article',
    excerpt: 'Discovery 阶段的服务识别需要关注误判：banner 伪装、honeypot、CDN 边缘会改变指纹。',
    page_no: null,
  ...
  },
  {
    ...
    asset_type: 'video_transcript',
    excerpt: '比赛中 nmap 服务识别误判把 MySQL 当成 PostgreSQL，应当再用 nmap -sV --version-all 复核。',
    page_no: null,
    chapter: 'CTF 服务识别复盘',
    timestamp: 412,
    reliability: 0.74,
...
const sdlEvidence: CourseMockEvidence = [
  {
    ...
    rights_note: 'CC BY-SA 4.0',
    asset_type: 'web_article',
    excerpt: '输入校验应当采用白名单：明确允许的字符集、长度与编码，再决定其它字段如何放行。',
    page_no: null,
    chapter: 'C5 校验输入',
    timestamp: null,
    reliability: 0.93,
  ...
  },
  {
    ...
    asset_type: 'web_article',
    excerpt: 'SBOM 至少需要包含组件名称、版本、唯一标识、依赖关系与作者，便于供应链漏洞响应。',
    page_no: null,
    chapter: 'SBOM 字段最小集',
    timestamp: null,
    reliability: 0.91,
  ...
  },
  {
    ...
    asset_type: 'repo_doc',
    excerpt: 'Java 反序列化审计检查项：禁止 ObjectInputStream 直读外部输入，开启 JEP 290 过滤器，记录类白名单。',
    page_no: null,
