...
- **最后更新时间**：2026-07-16（PR #51 的 T0～T7 真实功能补强已合并；提交 `868b4904` 收敛资源生成预算、主题、终态与 durable cancel；新增 WEBSEC-101 前端 `curated-demo` 课程体验，未改变 Runtime、9 Agent 或 Swiss v1 视觉契约）。原始版本 2026-06-05（由 Claude Opus 4.7 基于 4 份外部文档 + 仓库代码生成）。
...
## 0.1 最新工程铁律补充（来自 dev，必须与下文同时遵守）

> 本节保留 `dev` 分支中较新的工程约束。若与下文长版叙事发生表述差异，以本节和 §3 / §19 的保守约束为准；若下文提供更细上下文，则同时参考。

1. **The 9 agents are fixed** — never add a 10th, never rename, never drop. 固定清单：
   ...
   A3 新能力一律作为既有 agent 的新 skill，不注册新 agent。
2. **Cross-cutting infra is not an agent.** `rag/`、`knowledge/loaders/`、`services/knowledge/crawling/`、`services/storage/`、`runtime/router.py`、`runtime/guardrails/`、`runtime/harness/` 都是 middleware / framework，不得注册成 `crawler_agent` / `media_agent` / `spider_agent` 等第 10 个 agent。
3. **所有 domain 共用统一知识资产层。** `documents` + `document_assets` + `chunks` + `knowledge_nodes` + `knowledge_edges` 按 `domain` 过滤。禁止新增 `course_chunks` / `fund_chunks` / `policy_chunks` / `bilibili_chunks` / `zhihu_chunks` / platform_chunks 并列表。所有生成物落 `generated_resources` + `storage_objects`，不写回 `documents`。
4. **画像唯一源。** `user_profiles` 存 merged persona，`user_capabilities` 存可评估能力分。禁止 feature-local persona 表；更新链路为 `outcome_evaluator.update_capability` → `user_capabilities` → `career_planner.update_persona` → `user_profiles`。
5. **新增 endpoint / service / repository 文件必须有状态注释**：`# Status: real` / `# Status: mock` / `# Status: partial-real` / `# Status: planned`。
6. **生成式 skill 必须先 `rag.retrieve()`，再走质量闸。** RuntimeEngine/SkillExecutor 链路：`create step attempt + running agent_run → validate input → rag.retrieve → evidence snapshot → ContextBuilder → Provider journal/call → strict parse → Candidate Output`；QualityCheck 只能由后续的**显式 Workflow Node**独立执行，随后才可进入 Artifact Saga。
7. **每个真实 Skill 调用必须有且仅有一个 durable `agent_runs` child。** RuntimeEngine 在调用前创建 running child，SkillExecutor 负责 fenced 状态收敛；Skill 不得自行绕过该记录器或重复写入。
8. **修改本节、§3 铁律、§8 schema、采集策略、Harness contract、§19 差异说明时，必须同步更新 `CLAUDE.md` 与 `.codex/AGENTS.md`。**

### 0.1.1 多源采集合规

Scrapling、MediaCrawler、MindSpider 都是外部工具 / 数据采集参考，不是 agent。它们进入 SecureHub 的路径统一为：

`source_normalizer → storage_objects → documents + document_assets → chunks → knowledge_nodes/edges`

所有记录必须携带 `platform / source_url / author / published_at / fetched_at / license / rights_note`。禁止登录绕过、CAPTCHA 绕过、Cloudflare / 反爬绕过、代理池规避、大规模并发抓取、批量重托管版权内容。
...
| Storage | Local provider + **Tencent COS**（`anshu-skq-1385633904` / `ap-beijing` / private read-write；2026-07-12 Runtime upload/head/download/signed URL/delete 五步 smoke 通过；GitHub 外 data 全量同步仍独立未完成） |
| Agent runtime | **RuntimeEngine + SecureHub StateMachine**（唯一 production execution/state authority）；LangGraph 仅是可替换 TopologyAdapter，不能拥有 production state、checkpoint、worker scheduling 或 terminal state |
...
### 0.1.3 当前阶段与统一语言规范（2026-07-16）

> 本节用于修正 2026-07-08 前后文档中的阶段漂移。后续回答、任务提示词、PR 描述均按本节措辞执行。

| 维度 | 统一口径 |
|---|---|
| 当前阶段 | **Runtime v1.1 Wave 0-6 已签收，A3-S5~S7 均为 `real-accepted`**；PR #51 已合并 T0～T7 真实功能补强，不再把 Prompt 13 写成当前待执行排期。提交 `868b4904` 是合并后的资源生成可靠性修复；S8 仍为 `planned` + `external-gate-open`，Spark Gate 保留且暂缓 |
...
| 多课程 Catalog v1.0 | `courses` 的固定四课程真实目录：`WEBSEC-101` 是唯一 `ready` 完整课程；`CRYPTO-101` / `NET-SEC-201` / `SDL-201` 是 `preview` 目录记录。preview 仅显示预置内容，`detail/graph/path/progress` 为空投影，所有真实课程 workflow 在 root 前 `409 COURSE_CONTENT_NOT_READY`；未知 course 不回落 Web 安全 |
...
| 资源生成可靠性 | `resource_generate_v1` 将单次 Provider completion cap 与累计节点预算分离，只允许首次生成 + 一次有界返工；QualityCheck 缺陷写入 durable checkpoint，`kp_id` 由后端校验 course/domain 后生成权威主题 query。前端真实取消调用 `WorkflowRunClient.cancel(runId)`；失败/取消保留上一版 ready artifact，只有 root succeeded 才原子切换新版本 |
| PPT 视觉契约 | 提交 `00457ae0` 的 `securehub_swiss_v1` schema/prompt/fixture/renderer 未被 `868b4904` 改写；“项目学习资源 ppt”仍与竞赛提交 Deck 分属不同范围 |
...
- Wave 0-3 时真实 COS `put_object` 曾返回 `451 UnavailableForLegalReasons`；该历史失败不得改写。2026-07-12 独立 Runtime smoke 已在相同五步语义下成功，因此 451 不再是当前 COS Runtime 阻塞；local Artifact Saga 覆盖仍有效。
...
### 0.1.11 资源生成终态与 PPT 契约（2026-07-16）

- `NodeDefinition.provider_max_tokens` 是可选的单次 Provider completion cap；未设置时不进入 definition serialization，旧 workflow digest 与旧节点预算语义保持不变。`resource_generate_v1.producer` 使用单次 `2400`、累计 `8000`，配合 `max_rework_attempts=1` 只允许首次 + 一次返工。
- 返工必须消费持久化的 QualityCheck defect code/message；资源 query 必须由服务端按 `kp_id -> KnowledgeNode.name` 校验课程与 domain 后规范化。不得用通用标题或前端文本主导 RAG，也不得为消除失败而放宽显式 QualityCheck。
- 生成 attempt 与上一版 ready artifact 分离。pending artifact 只有在 root `succeeded` 后才能替换；`failed/blocked/cancelled` 必须结束 loading 并保留上一版。取消按钮调用 durable cancel API；SSE unsubscribe 只负责视图断连。
- `868b4904` 的定向/兼容验证为 64 passed，前端 typecheck/build 通过并经项目负责人手工核验。该结论只关闭已复现的预算、主题漂移和终态投影误失败；真实网络故障、Evidence 不足、安全拒绝仍须诚实失败，且不等于 Spark Gate 关闭。

### 0.1.12 WEBSEC-101 课程整理演示（2026-07-16）

- 仅 `WEBSEC-101` 在 `/course` 的结构化视图中扩展为六个同级 tab：课程入口、学习路径、资源工作台、题库与试卷、辅导对话、效果评估。`CRYPTO-101`、`NET-SEC-201`、`SDL-201` 仍是受治理的 `preview` 课程，绝不复用 WebSec 的路线、题目、资源或 URL 参数。
- 题库、三套试卷、三条课程路线和七类课程资料属于**显式 `curated-demo` / `external-preview` 内容**：用于答辩演示、浏览器本地复盘与课程导航，不得表述为后端实时生成、持久题库发布结果或新的 Runtime 验收。真实生成资源继续使用既有“已生成资源”视图。
- 六条 Bilibili 视频只保留原平台外链；其公开页面声明的标题与封面在明确授权后通过 `frontend/scripts/fetch-websec-bilibili-covers.py` 一次性写入 `frontend/public/assets/websec/bilibili/`。脚本不携带 Cookie、不下载视频/音频/弹幕，前端不得嵌入 iframe、播放/转码视频或在运行时请求 Bilibili 元数据。
- `CourseStudy` 必须把题库、路线图和课程目录视为可选模块：按需 `lazy` 加载并用局部 ErrorBoundary 显示可见降级，入口只依赖稳定的 `WEB_SECURITY_COURSE_CODE` / `WEB_SECURITY_COURSE_ID`。任何 `localStorage` 读写均须容错，存储失败时 URL 参数仍是视图权威，不能白屏。
...
### 3.3 ❌ 不要为新 domain 单独建知识库表（统一知识资产层）

所有 domain（`course_websec` / `course_crypto` / `course_network_security` / `course_secure_development` / `policy` / `fund` / `job` / `competition` / `paper`）共用**同一对表**：`documents` + `chunks`，以 `domain` 字段区分；`chunks.metadata`（JSONB）承载 domain-specific 字段。后三个课程 domain 当前只是 Catalog 受治理占位，尚未开放资料入库、RAG 或 Skill applicable-domain。
**禁止**新建 `course_chunks` / `fund_chunks` 等并列表。

### 3.4 ❌ 不要在 feature 模块自建画像（画像跨模块共享）

`user_profiles` 是**全平台唯一画像源**。任何模块需要画像数据时，**直接读 `user_profiles.dimensions`**；更新画像统一通过 `outcome_evaluator.update_capability` + `career_planner.update_persona` 两个 skill 完成。
...
> 本节保留早期仓库形态用于追溯，其中 `[planned]` 不代表 2026-07-12 当前状态。当前 Runtime/Agent/LLM/RAG/DB/Streaming 目录以代码、§0.1.3 与 §19.12 为准。
...
| `path` 学习路径 | 真实个性化路径；`WEBSEC-101` 可切换至课程整理的路线图 | `LearningPathDAG` + `LearningPathWorkspace` |
| `workbench` 资源工作台 | 已生成资源；`WEBSEC-101` 可切换课程资料与本地封面公开视频目录 | `ResourceTabs` + `CourseResourceWorkspace` |
| `exam` 题库与试卷 | `WEBSEC-101` 的课程整理题库、试卷蓝图、客观题自动计分和主观题自检 | `features/course/websec/WebSecurityExam.tsx` |
...
- $T_{a}$ = `kg.kp_prerequisites`, `llm.xfyun`
...
### 9.1 完整 schema（直接抄自 §5.2 规划文档，并补 SQLAlchemy 模型文件路径）

```sql
-- 通用文档与切片（所有 domain 共用）
documents (
  id UUID PRIMARY KEY,
  domain VARCHAR,                  -- course_websec / policy / fund / job / competition / paper
  source_type VARCHAR,
  title TEXT, url TEXT,
  raw_text TEXT,
  metadata JSONB,
  trust_score FLOAT,               -- 来源可信度 T(s)
  fetched_at TIMESTAMP
);
-- SQLAlchemy: backend/app/db/models/document.py
...
-- SQLAlchemy: backend/app/db/models/chunk.py
...
-- SQLAlchemy: backend/app/db/models/knowledge_point.py

kp_prerequisites (
  kp_id UUID REFERENCES knowledge_points,
  prereq_kp_id UUID REFERENCES knowledge_points,
  PRIMARY KEY (kp_id, prereq_kp_id)
);
-- SQLAlchemy: backend/app/db/models/kp_prerequisite.py

quiz_items (
  id UUID PRIMARY KEY,
  kp_id UUID REFERENCES knowledge_points,
  type VARCHAR,                    -- single_choice / multi_choice / fill / short_answer / code
  question TEXT,
  options JSONB,
  answer TEXT,
  difficulty INT,
...
-- SQLAlchemy: backend/app/db/models/quiz_item.py

users (
  id UUID PRIMARY KEY,
  ...
);
-- SQLAlchemy: backend/app/db/models/user.py

user_profiles (
  user_id UUID PRIMARY KEY,
  dimensions JSONB,                -- 6+ 维度都在这里
...
learning_events (
  id UUID PRIMARY KEY,
  user_id UUID,
  event_type VARCHAR,              -- view_doc / quiz_correct / quiz_wrong / complete_lab
  kp_id UUID,
  resource_id UUID,
  result JSONB,
  occurred_at TIMESTAMP
);
-- SQLAlchemy: backend/app/db/models/learning_event.py
...
-- SQLAlchemy: backend/app/db/models/agent.py
...
-- SQLAlchemy: backend/app/db/models/agent_skill.py
...
agent_runs (
  ...
  user_id UUID,
  agent_id UUID REFERENCES agents,
  skill_id UUID REFERENCES agent_skills,
  parent_run_id UUID REFERENCES agent_runs,
  input_summary JSONB,
...
CREATE INDEX ON agent_runs (workflow_name, created_at DESC);
CREATE INDEX ON agent_runs (user_id, created_at DESC);
-- SQLAlchemy: backend/app/db/models/agent_run.py
```
...
| 索引 | 作用 |
| --- | --- |
| `chunks USING ivfflat (embedding vector_cosine_ops)` | pgvector 近似最近邻，向量检索关键 |
| `documents(domain)` + `chunks(domain)` | 多 domain 过滤入口 |
| `agent_runs(workflow_name, created_at DESC)` | trace 可视化按工作流倒序 |
| `agent_runs(user_id, created_at DESC)` | 个人 trace 查询 |
| `quiz_items(kp_id)` | 题库按知识点取 |
...
> 2026-07-11 更新：Provider 家族与 Qwen embedding 仍为已生产化前提；v1.1 Wave 0-3 已完成唯一 RuntimeEngine/Durable Core/五条产品路径验收。下文所有较旧的 LangGraph production-runtime 表述均被本节和 §0.1.3/§19.11 覆盖。
...
| 事件类型 | 时机 | data 内容 |
| --- | --- | --- |
| `progress` | root/node 状态变化 | `{node_id, status, agent_id, skill_id, percentage}` |
| `evidence` | real RAG snapshot 已持久化 | `{items, evidence_snapshot_ids}` |
| `token` | LLM 流式 token | `{content, step_attempt_id, provider_call_id, stream_attempt}` |
| `artifact` | active artifact 已持久化 | `{resource_id, resource_type, title, object_key?}` |
| `trace` | child/Provider/QualityCheck 状态 | `{agent, skill, step_attempt_id, actual_provider, actual_model}` |
| `done` | terminal succeeded/cancelled | `{run_id, final_output_ref, quality_score}` |
| `error` | recoverable 或 terminal failure/blocked | `{code, message, recoverable}` |

每个 frame 同时携带 SSE `id` 与 JSON `sequence`。客户端按 `(workflow_run_id, sequence)` 去重，以 `Last-Event-ID` replay，并在 Redis live gap 时从 PostgreSQL 补齐；terminal `done` 与 terminal `error` 互斥，之后不得再发 token/artifact。
...
| 4 | **是否引入 Neo4j 做知识图谱** | **不引入**，用 `knowledge_points + kp_prerequisites` 两张关系表即可 | Neo4j 部署成本高；本项目图规模小（50–100 节点），关系表足够 |
...
- **理由**：A3 阶段是工程噪声；pgvector 性能足够 500–5000 chunks 规模；知识图谱用关系表 `kp_prerequisites` 替代 Neo4j；BM25 用 `tsvector` 而非 Elasticsearch；文档说明"生产架构已规划"即可
...
### 19.8 A/B/C 真实联调阶段历史口径（2026-07-09，已被 §19.12 覆盖）
...
### 19.10 Agent Run API 真实闭环签收（2026-07-10）
...
- **历史边界**：本节只描述 2026-07-10 Agent Run API 专项；当时尚未验收的五条产品路径已由后续 Wave 0-6 和 2026-07-12 live handoff 解除，当前状态以 §19.12 为准。
- **阅读收敛**：涉及该专项时先读
  `Plan/2026-07-10_Agent_Run_API_真实闭环凝练索引.md`，再按其 source:line 追证，不默认逐份读
  Agent-Run 原始 Plan / Prompt / Workout。

### 19.11 Agent Runtime v1.1 Wave 0-3 签收（2026-07-11）
...
- **当时外部阻塞（历史）**：Wave 0-3 验收时真实 COS `put_object` 返回 `451 UnavailableForLegalReasons`。该失败记录保留；2026-07-12 COS Runtime 五步成功由 §19.12 覆盖当前状态。
...
### 19.12 Agent Runtime v1.1 Wave 4-6 与 Live Handoff 签收（2026-07-11 至 2026-07-12）
...
2. **创建 framework-neutral 定义**：`backend/app/runtime/workflows/<scene>_<action>_v1.py`，以 `WorkflowDefinition` / `NodeDefinition` 表达节点和边；不得把 LangGraph state/checkpoint 写入生产契约。
3. **注册版本化 workflow**：经 `workflow_registry.py` 交给唯一 `RuntimeEngine`；节点只能引用固定 Agent/Skill 或显式 deterministic Workflow Action。
4. **每个生成节点必须**：走 `SkillExecutor` 的 RAG/evidence/context/provider/strict-parse 链；QualityCheck 为独立 node，timeout/retry/rework/cancel 由 RuntimeEngine 持久化管理。
5. **创建对应 endpoint adapter**：只做 DTO/auth/input mapping，经 `WorkflowApplicationService` 创建唯一 root；不得 import Skill、创建 task/queue 或随机 run ID。
6. **加 SSE/前端契约**：沿用 §11.1 七类 `EventEnvelope`、Last-Event-ID 和 gap replay；断连不得取消 durable run。
7. **加集成与故障测试**：`backend/tests/runtime/test_<workflow>.py`，至少覆盖 state/error/real-fixture 隔离；若落 artifact/provider，再覆盖其恢复语义。
8. **同步 `docs/api/workflow-run-contract.md`、本文件和 `.codex/AGENTS.md`**。
...
7. **若需流式输出**：用 `WorkflowRunClient` 监听 SSE，按 §11.1 七类事件、Last-Event-ID、gap replay 和 stream replacement 处理
8. **更新本文档 §4 目录树（如果加了新文件）+ §5.2 路由表（如果加了路由）**
9. **若向 `/course` 接入可选演示模块**：稳定入口不得静态依赖它；使用 `React.lazy` + 局部 ErrorBoundary，URL 参数必须可在浏览器存储失效时独立恢复视图。
10. **若展示外部公开视频封面**：运行时只读取已提交的本地静态资源并保留外链；公开页面采集只能放在显式脚本中、白名单化、无 Cookie、无视频下载，不能以浏览器 `fetch` 代替后端或绕过跨域限制。
