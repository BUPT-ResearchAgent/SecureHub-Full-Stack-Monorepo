...
| `GET /api/v1/teacher/production/courses/{course_id}/assets?include_deleted=` | 读取本人课程资产治理状态（可显式包含软删除） | 只投影既有 `documents` / `document_assets` 与治理生命周期，供刷新后一致的教师入口读取。 |
| `POST /api/v1/teacher/production/courses/{course_id}/assets` | 绑定统一知识资产 | 仅绑定既有 `documents` 与匹配的 `document_assets`，写入 `course_document_bindings`、`course_asset_governance`。 |
...
| `GET /api/v1/teacher/production/courses/{course_id}/weakness-snapshots` | 读取本课程已保存的薄弱点快照 | 仅返回当前教师有权限的可复现聚合，不重新计算或复制画像。 |
...
| `GET /api/v1/teacher/production/courses/{course_id}/teaching-recommendations` | 读取持久化建议及处置状态 | 仅投影 versioned diff、Evidence 引用和状态；不会自动写回课程。 |
...
| `GET /api/v1/teacher/production/courses/{course_id}/assignments` | 读取本课程已布置的版本化作业 | 返回 `assessment` / `version` / `assignment` 的受权投影，刷新后不依赖 mock 常量。 |
...
| `GET /api/v1/teacher/production/assessment-assignments/{assignment_id}/submissions` | 教师读取该作业的真实提交与成绩决定 | 仅作业所属教师可读；AI 建议与教师覆盖/发布状态均来自持久化 grade decision。 |
