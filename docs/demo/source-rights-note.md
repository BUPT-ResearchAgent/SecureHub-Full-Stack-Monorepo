...
| 知乎 MediaCrawler export 2026-07-08 | Consume 18 manually provided Zhihu answer export rows for `course_websec` retrieval, storing title, answer text, source link, author display name, and redacted source JSON. | Use only for learning and competition demo evidence. Do not retain `user_id`, avatar URL, user token, or profile link. Do not batch rehost Zhihu UGC. |
| 小红书 MediaCrawler export 2026-07-08 | Consume manually provided XHS note export rows for `course_websec` retrieval, storing title, note text, source link, author display name, metrics, and redacted source JSON. | Use only for learning and competition demo evidence. Do not download images/videos, do not retain `user_id`, avatar URL, `xsec_token`, IP location, or signed media URLs for redistribution. |
| MindSpider reference | P2 reference-only material for process comparison; fixture topics only. | Do not run MindSpider, do not connect to the production crawling chain, do not add an agent, and do not add platform-specific tables. |
